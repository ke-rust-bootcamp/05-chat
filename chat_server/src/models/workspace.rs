...
mod tests {
    ...
    async fn workspace_should_create_and_set_owner() -> Result<()> {
        ...
        let input = CreateUser::new(&ws.name, "C Chen", "cc@acme.org", "Hunter42");
    ...
    #[tokio::test]
    async fn workspace_fetch_all_chat_users() -> Result<()> {
        let (_tdb, pool) = get_test_pool(None).await;

        let users = Workspace::fetch_all_chat_users(1 as _, &pool).await?;
